...
t_color	compute_diffuse(t_vec normal, t_light light, t_vec light_dir)
{
	float	n_dot_l;
...
t_color	compute_specular(t_vec normal, t_light light, t_vec light_dir, t_ray *ray)
{
	t_vec	reflect_dir;
	t_vec	view_dir;
	float	spec_angle;

	reflect_dir = vec_sub(vec_scal(normal, 2 * vec_dot(normal, light_dir)), light_dir);
	reflect_dir = vec_norm(reflect_dir);
	view_dir = vec_scal(ray->dir, -1);
	spec_angle = fmaxf(vec_dot(reflect_dir, view_dir), 0.0);
	spec_angle = ft_pow(spec_angle, 64); //exponant = shininess
...
static t_color	compute_lights(t_scene *s, t_vec hit_point, t_vec normal, t_ray *r)
{
	...
	while (idx < s->lights_len)
	{
		...
		if (is_in_shadow(s, hit_point, normal, light))
		{
			idx++;
			continue;
		}
		light_dir = vec_norm(vec_sub(light.pos, hit_point));
		diffuse = compute_diffuse(normal, light, light_dir);
		specular = compute_specular(normal, light, light_dir, r);
