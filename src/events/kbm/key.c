...
/*   Updated: 2025/11/21 16:01:20 by mmichele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (c->s.controlled && ft_strchr("wasdc ijkluotgyh12345678", key) >= 0)
...
int	key_release(int key, struct s_ctx *c)
{
	...
	if (key == 'z')
		...
		return (1);
	else if (ft_strchr("p[]", key) >= 0)
		return (full_render(c, ft_strchr("p[]", key)));
